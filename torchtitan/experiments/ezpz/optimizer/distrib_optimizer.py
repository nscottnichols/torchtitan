...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def build_model_gbuf_param_range_map(cls, model, dtype, gbuf_world_range):
        ...
        for param, param_world_indexes in param_world_index_map.items():
            ...
            if param_local_end > param_local_start:
                ...
                sub_param_start = max(
                    0, gbuf_world_range.start - param_world_start
                )
    ...
    def build_model_gbuf_range(cls, model, dtype):
        ...
        max_gbuf_range_size = int(
            math.ceil(gbuf_size / data_parallel_world_size)
        )
        ...
        for r in range(data_parallel_world_size):
            ...
            gbuf_world_end = min(
                gbuf_size, gbuf_world_start + max_gbuf_range_size
            )
    ...
    def build_model_param_gbuf_map(cls, model_gbuf_ranges):
        ...
        for model_index, model_gbuf_range_map in enumerate(model_gbuf_ranges):
            for dtype, gbuf_range_map in model_gbuf_range_map.items():
                for param, param_range_map in gbuf_range_map[
                    "param_map"
                ].items():
        ...
        for group_index, group_range in enumerate(opt_group_ranges):
            ...
            for model_param in group_range["params"]:
                ...
                if model_param.type() in [
                    ...
                    shard_fp32_from_float16_params_this_group.append(
                        shard_main_param
                    )
    ...
    def __init__(
        ...
        for model_index, model in enumerate(self.models):
            self.model_gbuf_ranges.append(
                self.build_model_gbuf_range_map(model)
            )
        ...
        state_dict_param_groups = [
            {
                **group,
                "params": list(
                    inner_state_dict["param_groups"][idx]["params"]
                ),
            }
    ...
    def reduce_model_grads(self, args, timers):
        ...
        for index, (model_index, dtype, gbuf, gbuf_views) in enumerate(
            gbuf_view_items
        ):
    ...
    def gather_model_params(self, args, timers):
        ...
        for index, (model_index, dtype, pbuf, pbuf_views) in enumerate(
            pbuf_view_items
        ):
    ...
    def _copy_model_grads_to_main_grads(self):
        ...
        def copy_group_grads(model_groups, shard_main_groups):
            for model_group, shard_main_group in zip(
                model_groups, shard_main_groups
            ):
                for model_param, shard_main_param in zip(
                    model_group, shard_main_group
                ):
                    param_range_map = self.get_model_param_range_map(
                        model_param
                    )
    ...
    def _copy_main_params_to_model_params(self):
        ...
        def copy_group_params(shard_main_groups, model_groups):
            for shard_main_group, model_group in zip(
                shard_main_groups, model_groups
            ):
                for shard_main_param, model_param in zip(
                    shard_main_group, model_group
                ):
                    param_range_map = self.get_model_param_range_map(
                        model_param
                    )
