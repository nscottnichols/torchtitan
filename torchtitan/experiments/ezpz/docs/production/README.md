...
| 80B | 4 | **10** (smoke) | **12.03** | smoke | [`int-r7`](agpt/80b/n4/README.md) ✅ end-to-end validated; **256N blocked on NaN** | **NaN root cause identified 2026-06-12: XPU nondeterministic-op output accumulates into bf16 forward overflow at GBS≥192**, not bf16 alone or TP=2 loss-reduction. `--debug.deterministic` (per [CLAUDE.md](../../.claude/CLAUDE.md) — NEVER `_warn_only`) validated clean at n=32 in 8539896 + 8540019; n=64/128/256 sweep in progress (n=64 `8540102` R now with overprovisioning + failover). **Recommended production fix: `--debug.deterministic` (~50% throughput cost) over fp32-activations at TP=4 (~75% cost).** See [80b-n32-nan-diagnosis.md](../experiments/agpt/aurora/20260611-80b-n32-nan-diagnosis.md). |
