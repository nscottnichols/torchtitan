...
CKPT_INTERVAL=100
CKPT_DIR="checkpoints/${FLAVOR}-${MODEL}-${OPTIMIZER}-${DFL_NAME}-n${NNODES}-gbs${GBS}"

DATA_CACHE_PATH="${CKPT_DIR}/.cache/${DFL_NAME}/index-cache"

log_message INFO "==========================================="
log_message INFO "Training ${FLAVOR} ${MODEL} on ${TRAIN_TOKENS} tokens"
log_message INFO "-------------------------------------------"
...
log_message INFO "Expert parallel degree (EP): ${EP}"
