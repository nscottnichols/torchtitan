...
# Smoke test for the agpt-2b-v2 clone:
# - validates the new torch 2.13 + xpu venv builds and runs end-to-end
# - validates the fp32-master fix actually unsticks RMSNorm.weight
# - 2 nodes, 50 steps, one checkpoint saved at the end (no compile)
#
# Post-run check:
#   python3 -c "
#   import torch, torch.distributed.checkpoint as dcp
#   sd = {'norm.weight': torch.zeros(2048)}
#   dcp.load(sd, checkpoint_id='outputs/checkpoints/agpt-2b-sophiag-olmo-mix-1124-n2-gbs48-smoke/step-50')
#   print('norm.weight changed?', not torch.allclose(sd['norm.weight'], torch.ones(2048)))
#   "
...
LBS="${LBS:-1}"  # smoke: 1 to leave headroom (logits w/ vocab=256k consume ~16GB at LBS=2)
GBS=$(( NGPUS * LBS ))   # TP=PP=CP=GAS=1
